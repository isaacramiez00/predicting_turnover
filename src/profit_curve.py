...
def get_train_test(filepath):
    ...
    turnover_df = make_clean_churn_df(raw_df)
    y = turnover_df.pop('left').values
    X = turnover_df.values
...
def make_clean_churn_df(df):
    ...
    df["salary"] = df["salary"].astype('category').cat.reorder_categories(salary_col).cat.codes
    salary_code = df.salary.value_counts()
    ...
    df["department"] = df["department"].astype('category').cat.reorder_categories(department_col).cat.codes
...
if __name__ == '__main__':
    turnover_filepath = '../data/cleanedturnover.csv'
    cost_benefit = np.array([[50000, 0], [50000, 0]]) # report back to expenses of employee turnover

    models = [RF(), LR(), GBC(), SVC(), KNeighborsClassifier()]

    X_train, X_test, y_train, y_test = get_train_test(turnover_filepath)
    standard_confusion_matrix(y_test, y_train)
    profit_curve(cost_benefit, pre)

    profit_curve_main(turnover_filepath, cost_benefit)
    sampling_main(LR(), turnover_filepath, cost_benefit)